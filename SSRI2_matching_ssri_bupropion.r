...
system(paste("gsutil cp", paste0(bucket,"/data/","data_ssri_bupropion.csv"), "data_ssri_bupropion.csv"))
data <- read.csv("data_ssri_bupropion.csv")
...
for(disease in diseases){
    ...
    prs <- read.csv(paste0(disease,'_scores.csv'))[c("s","score")]
...
write.csv(data, "data_ssri_bupropion_scored.csv")
system(paste("gsutil cp", "data_ssri_bupropion_scored.csv", paste0(bucket,"/data/","data_ssri_bupropion_scored.csv")))
...
write.csv(x = sample, file = "sample_ssri_bupropion_scored.csv")

bucket <- Sys.getenv("WORKSPACE_BUCKET")
local_file_path <- file.path("sample_ssri_bupropion_scored.csv")
remote_file_path <- file.path(bucket, "data", "sample_ssri_bupropion_scored.csv")
