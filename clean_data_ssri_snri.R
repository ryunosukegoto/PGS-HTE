...
clean_data_ssri_snri <- function(data,dataset_drug_df,dataset_condition_df,dataset_survey_df,drug_treatment,drug_control,seed,redo=FALSE) {
  
  datafile <- "clean_data_ssri_snri.Rda"
  ...
  id_snri <- unique(dataset_drug[dataset_drug[[drug_control]]==1,]$person_id)
  
  data_date_drug_snri <- dataset_drug[
    person_id %in% id_snri & snri == 1,
    .SD[which.min(drug_exposure_start_date)], by = person_id
  ]
  ...
  data_snri <- data.table(
    person_id = data_date_drug_snri$person_id,
    date_drug = data_date_drug_snri$drug_exposure_start_date
  )
  
  # Perform a join to get both dates
  merged_data <- merge(data_ssri, data_snri, by = "person_id", all=TRUE, suffixes = c("_ssri", "_snri"))
  
  data_ssri <- data_ssri[person_id %in% merged_data[date_drug_ssri < date_drug_snri | is.na(date_drug_snri)]$person_id]
  data_snri <- data_snri[person_id %in% merged_data[date_drug_snri < date_drug_ssri | is.na(date_drug_ssri)]$person_id]
  
  id_ssri <- data_ssri$person_id
  id_snri <- data_snri$person_id
  ...
  suppressWarnings({
    ...
    # Merge with survey data to get smoking status
    smoker_data <- dataset_survey[
      person_id %in% id_snri & question == "Smoking: 100 Cigs Lifetime",
      .(person_id, answer)]
    ...
    data_snri <- merge(data_snri, smoker_data[, .(person_id, smoker)], by = "person_id", all.x = TRUE)
    
    
    # Merge with survey data to get income
    income_data <- dataset_survey[
      person_id %in% id_snri & question == "Income: Annual Income",
      .(person_id, answer)]
    ...
    data_snri <- merge(data_snri, income_data[, .(person_id, income)], by = "person_id", all.x = TRUE)
    
    # Merge with survey data to get insurance
    insurance_data <- dataset_survey[
      person_id %in% id_snri & question == "Insurance: Health Insurance",
      .(person_id, answer)]
    ...
    data_snri <- merge(data_snri, insurance_data[, .(person_id, insurance)], by = "person_id", all.x = TRUE)
    
    # Merge with survey data to get edu
    edu_data <- dataset_survey[
      person_id %in% id_snri & question == "Education Level: Highest Grade",
      .(person_id, answer)]
    ...
    data_snri <- merge(data_snri, edu_data[, .(person_id, edu)], by = "person_id", all.x = TRUE)
    
    # Merge with survey data to get birthplace
    birthplace_data <- dataset_survey[
      person_id %in% id_snri & question == "The Basics: Birthplace",
      .(person_id, answer)]
    ...
    data_snri <- merge(data_snri, birthplace_data[, .(person_id, birthplace)], by = "person_id", all.x = TRUE)
    ...
    #sui
    sui_data <- dataset_condition[person_id %in% id_snri & sui == 1]
    
    sui_info <- sui_data[, .(
      sui_dates = list(condition_start_date)
    ), by = person_id]
    
    sui_info <- merge(sui_info, data_snri[,.(person_id,date_drug)], by = "person_id", all.x=TRUE)
    ...
    # Merge with the main data_snri table
    data_snri <- merge(data_snri, sui_info[,!("date_drug")], by = "person_id", all.x = TRUE)
    
    data_snri[, sui := fifelse(is.na(sui), 0, sui)]
    data_snri[, sui_3 := fifelse(is.na(sui_3), 0, sui_3)]
    data_snri[, sui_5 := fifelse(is.na(sui_5), 0, sui_5)]
    data_snri[, sui_pmh := fifelse(is.na(sui_pmh), 0, sui_pmh)]
    data_snri[, sui_time := fifelse(is.na(sui_time), as.numeric(as.Date("2022-07-01", origin = "1970-01-01") - date_drug), sui_time)]
    
    rm(sui_data, sui_info)
    
    
    #anxiety
    anxiety_data <- dataset_condition[person_id %in% id_snri & anxiety == 1]
    
    anxiety_info <- anxiety_data[, .(
      anxiety_dates = list(condition_start_date)
    ), by = person_id]
    
    anxiety_info <- merge(anxiety_info, data_snri[,.(person_id,date_drug)], by = "person_id", all.x=TRUE)
    ...
    # Merge with the main data_snri table
    data_snri <- merge(data_snri, anxiety_info[,!("date_drug")], by = "person_id", all.x = TRUE)
    
    data_snri[, anxiety := fifelse(is.na(anxiety), 0, anxiety)]
    data_snri[, anxiety_3 := fifelse(is.na(anxiety_3), 0, anxiety_3)]
    data_snri[, anxiety_5 := fifelse(is.na(anxiety_5), 0, anxiety_5)]
    data_snri[, anxiety_pmh := fifelse(is.na(anxiety_pmh), 0, anxiety_pmh)]
    data_snri[, anxiety_time := fifelse(is.na(anxiety_time), as.numeric(as.Date("2022-07-01", origin = "1970-01-01") - date_drug), anxiety_time)]
    ...
    #alcohol
    alcohol_data <- dataset_condition[person_id %in% id_snri & alcohol == 1]
    
    alcohol_info <- alcohol_data[, .(
      alcohol_dates = list(condition_start_date)
    ), by = person_id]
    
    alcohol_info <- merge(alcohol_info, data_snri[,.(person_id,date_drug)], by = "person_id", all.x=TRUE)
    ...
    # Merge with the main data_snri table
    data_snri <- merge(data_snri, alcohol_info[,!("date_drug")], by = "person_id", all.x = TRUE)
    
    data_snri[, alcohol := fifelse(is.na(alcohol), 0, alcohol)]
    data_snri[, alcohol_3 := fifelse(is.na(alcohol_3), 0, alcohol_3)]
    data_snri[, alcohol_5 := fifelse(is.na(alcohol_5), 0, alcohol_5)]
    data_snri[, alcohol_pmh := fifelse(is.na(alcohol_pmh), 0, alcohol_pmh)]
    data_snri[, alcohol_time := fifelse(is.na(alcohol_time), as.numeric(as.Date("2022-07-01", origin = "1970-01-01") - date_drug), alcohol_time)]
    
    rm(alcohol_data, alcohol_info)
    
    
    #substance
    substance_data <- dataset_condition[person_id %in% id_snri & substance == 1]
    
    substance_info <- substance_data[, .(
      substance_dates = list(condition_start_date)
    ), by = person_id]
    
    substance_info <- merge(substance_info, data_snri[,.(person_id,date_drug)], by = "person_id", all.x=TRUE)
    ...
    # Merge with the main data_snri table
    data_snri <- merge(data_snri, substance_info[,!("date_drug")], by = "person_id", all.x = TRUE)
    
    data_snri[, substance := fifelse(is.na(substance), 0, substance)]
    data_snri[, substance_3 := fifelse(is.na(substance_3), 0, substance_3)]
    data_snri[, substance_5 := fifelse(is.na(substance_5), 0, substance_5)]
    data_snri[, substance_pmh := fifelse(is.na(substance_pmh), 0, substance_pmh)]
    data_snri[, substance_time := fifelse(is.na(substance_time), as.numeric(as.Date("2022-07-01", origin = "1970-01-01") - date_drug), substance_time)]
    
    rm(substance_data, substance_info)
    
    #schizophrenia
    schizophrenia_data <- dataset_condition[person_id %in% id_snri & schizophrenia == 1]
    
    schizophrenia_info <- schizophrenia_data[, .(
      schizophrenia_dates = list(condition_start_date)
    ), by = person_id]
    
    schizophrenia_info <- merge(schizophrenia_info, data_snri[,.(person_id,date_drug)], by = "person_id", all.x=TRUE)
    ...
    # Merge with the main data_snri table
    data_snri <- merge(data_snri, schizophrenia_info[,!("date_drug")], by = "person_id", all.x = TRUE)
    
    data_snri[, schizophrenia := fifelse(is.na(schizophrenia), 0, schizophrenia)]
    data_snri[, schizophrenia_3 := fifelse(is.na(schizophrenia_3), 0, schizophrenia_3)]
    data_snri[, schizophrenia_5 := fifelse(is.na(schizophrenia_5), 0, schizophrenia_5)]
    data_snri[, schizophrenia_pmh := fifelse(is.na(schizophrenia_pmh), 0, schizophrenia_pmh)]
    data_snri[, schizophrenia_time := fifelse(is.na(schizophrenia_time), as.numeric(as.Date("2022-07-01", origin = "1970-01-01") - date_drug), schizophrenia_time)]
    
    rm(schizophrenia_data, schizophrenia_info)
    
    #eating
    eating_data <- dataset_condition[person_id %in% id_snri & eating == 1]
    
    eating_info <- eating_data[, .(
      eating_dates = list(condition_start_date)
    ), by = person_id]
    
    eating_info <- merge(eating_info, data_snri[,.(person_id,date_drug)], by = "person_id", all.x=TRUE)
    ...
    # Merge with the main data_snri table
    data_snri <- merge(data_snri, eating_info[,!("date_drug")], by = "person_id", all.x = TRUE)
    
    data_snri[, eating := fifelse(is.na(eating), 0, eating)]
    data_snri[, eating_3 := fifelse(is.na(eating_3), 0, eating_3)]
    data_snri[, eating_5 := fifelse(is.na(eating_5), 0, eating_5)]
    data_snri[, eating_pmh := fifelse(is.na(eating_pmh), 0, eating_pmh)]
    data_snri[, eating_time := fifelse(is.na(eating_time), as.numeric(as.Date("2022-07-01", origin = "1970-01-01") - date_drug), eating_time)]
    
    rm(eating_data, eating_info)
    
    
    #ptsd
    ptsd_data <- dataset_condition[person_id %in% id_snri & ptsd == 1]
    
    ptsd_info <- ptsd_data[, .(
      ptsd_dates = list(condition_start_date)
    ), by = person_id]
    
    ptsd_info <- merge(ptsd_info, data_snri[,.(person_id,date_drug)], by = "person_id", all.x=TRUE)
    ...
    # Merge with the main data_snri table
    data_snri <- merge(data_snri, ptsd_info[,!("date_drug")], by = "person_id", all.x = TRUE)
    
    data_snri[, ptsd := fifelse(is.na(ptsd), 0, ptsd)]
    data_snri[, ptsd_3 := fifelse(is.na(ptsd_3), 0, ptsd_3)]
    data_snri[, ptsd_5 := fifelse(is.na(ptsd_5), 0, ptsd_5)]
    data_snri[, ptsd_pmh := fifelse(is.na(ptsd_pmh), 0, ptsd_pmh)]
    data_snri[, ptsd_time := fifelse(is.na(ptsd_time), as.numeric(as.Date("2022-07-01", origin = "1970-01-01") - date_drug), ptsd_time)]
    
    rm(ptsd_data, ptsd_info)
    
    
    #adhd
    adhd_data <- dataset_condition[person_id %in% id_snri & adhd == 1]
    
    adhd_info <- adhd_data[, .(
      adhd_dates = list(condition_start_date)
    ), by = person_id]
    
    adhd_info <- merge(adhd_info, data_snri[,.(person_id,date_drug)], by = "person_id", all.x=TRUE)
    ...
    # Merge with the main data_snri table
    data_snri <- merge(data_snri, adhd_info[,!("date_drug")], by = "person_id", all.x = TRUE)
    
    data_snri[, adhd := fifelse(is.na(adhd), 0, adhd)]
    data_snri[, adhd_3 := fifelse(is.na(adhd_3), 0, adhd_3)]
    data_snri[, adhd_5 := fifelse(is.na(adhd_5), 0, adhd_5)]
    data_snri[, adhd_pmh := fifelse(is.na(adhd_pmh), 0, adhd_pmh)]
    data_snri[, adhd_time := fifelse(is.na(adhd_time), as.numeric(as.Date("2022-07-01", origin = "1970-01-01") - date_drug), adhd_time)]
    ...
    # Process drug data: tca                           
    tca_data <- dataset_drug[person_id %in% id_snri & tca == 1]
    
    tca_info <- tca_data[, .(
      tca_dates = list(drug_exposure_start_date)
    ), by = person_id]
    
    tca_info <- merge(tca_info, data_snri[,.(person_id,date_drug)], by = "person_id", all.x=TRUE)
    
    tca_info[,tca_pre := fifelse(mapply(compare_dates_pre, tca_dates, date_drug), 1, 0)]
    
    # Merge with the main data_snri table
    data_snri <- merge(data_snri, tca_info[,!("date_drug")], by = "person_id", all.x = TRUE)
    
    data_snri[, tca_pre := fifelse(is.na(tca_pre), 0, tca_pre)]
  ...
  # Process demographic variables
  data_demographics_snri <- data_demographics[person_id %in% id_snri]
  
  data_demographics_snri[, date_of_birth := as.Date(substr(date_of_birth, 1, 10), origin = "1970-01-01")]
  
  data_snri <- merge(data_snri, data_demographics_snri[, .(
    person_id,
    date_of_birth,
    gender = case_when(
      gender %in% c("I prefer not to answer", "PMI: Skip", "Not man only, not woman only, prefer not to answer, or skipped") ~ NA,
  ...
  data_snri[, age := round(as.numeric(date_drug - date_of_birth)/365)]
  
  data_snri = as.data.frame(data_snri)
  
  data_snri$arm <- 0
  
  data_snri <- data_snri[c("person_id","date_drug","sui","sui_3","sui_5","sui_time","arm","gender","age","rande","edu","income","insurance","birthplace","smoker","sui_pmh","anxiety_pmh","alcohol_pmh","substance_pmh","schizophrenia_pmh","eating_pmh","ptsd_pmh","adhd_pmh","tca_pre")]
  
  print("Full snri data")                                                      
  print(summary(data_snri))
  
  
  data_snri <- data_snri[complete.cases(data_snri),] 
  
  print("Complete cases, snri")                                                      
  print(summary(data_snri))
  ...
  sample_match <- rbind(data_ssri,data_snri)
